...
<body>
    ...
    if (isset($_GET['id'])) {
        ...
        $article = $articleController->indexId($ma_bviet); // Gọi đúng phương thức
        ...
        if (isset($_FILES['hinhanh']) && $_FILES['hinhanh']['error'] == 0) {
            ...
            if (in_array($hinhanh['type'], $allowedTypes) && $hinhanh['size'] <= $maxFileSize) {
                ...
                if (move_uploaded_file($hinhanh['tmp_name'], $uploadFilePath)) {
                    // Gọi hàm update để cập nhật bài viết cùng với hình ảnh mới
                    $articleController->update($ma_bviet, $tieude, $ten_bhat, $ma_tloai, $tomtat, $noidung, $ma_tgia, $ngayviet, $uploadFilePath);
                    header("Location: article.php?success=1");
                    exit();
                } else {
                    echo "<div class='alert alert-danger'>Lỗi trong quá trình tải lên hình ảnh!</div>";
                }
            } else {
                echo "<div class='alert alert-danger'>Chỉ chấp nhận file ảnh (JPEG, PNG, GIF) và kích thước tối đa 2MB!</div>";
            }
        } else {
            // Không có file ảnh mới, giữ lại ảnh cũ
            $existingImagePath = $article['hinhanh'];
            $articleController->update($ma_bviet, $tieude, $ten_bhat, $ma_tloai, $tomtat, $noidung, $ma_tgia, $ngayviet, $existingImagePath);
            header("Location: article.php?success=1");
            exit();
        }
        ...
        <div class="col-sm">
            ...
            <div class="input-group mt-3 mb-3">
                ...
                <div class="input-group mt-3 mb-3">
                    ...
                    <input type="text" class="form-control" name="tieude" value="<?php echo htmlspecialchars($article['tieude']); ?>" >
                </div>
                <div class="input-group mt-3 mb-3">
                ...
                <div class="input-group mt-3 mb-3">
                    ...
                    <input type="text" class="form-control" name="ma_tloai" value="<?php echo htmlspecialchars($article['ma_tloai']); ?>" required>
                </div>
                <div class="input-group mt-4 mb-4">
                    <span class="input-group-text" id="lblCatName">Tóm Tắt </span>
                    <input class="form-control" name="tomtat" value="<?php echo htmlspecialchars($article['tomtat']); ?>"required>
                </div>
                <div class="input-group mt-3 mb-3">
                    <span class="input-group-text" id="lblCatName">Nội Dung</span>
                    <textarea class="form-control" name="noidung" value="<?php echo htmlspecialchars($article['noidung']); ?>"required></textarea>
                </div>
                <div class="input-group mt-3 mb-3">
                    <span class="input-group-text" id="lblCatName">Tác Giả</span>
                    <input type="text" class="form-control" name="ma_tgia" value="<?php echo htmlspecialchars($article['ma_tgia']); ?>">
                </div>
                <div class="input-group mt-3 mb-3">
